...
```bash
curl -X GET "http://localhost:8001/api/chat/chat?message=你好"
```

**带 URL 编码的消息**:
```bash
curl -X GET "http://localhost:8001/api/chat/chat?message=查询贵州茅台的K线数据"
```
...
```bash
# 查询股票代码
curl -X GET "http://localhost:8001/api/chat/chat?message=查询600519的股票信息"

# 查询K线数据
curl -X GET "http://localhost:8001/api/chat/chat?message=查询sh.600519最近5天的数据"

# 查询所有股票
curl -X GET "http://localhost:8001/api/chat/chat?message=帮我列出所有股票"
```
...
```bash
curl -N -X GET "http://localhost:8001/api/chat/chat-stream?message=你好"
```

**带 URL 编码的消息**:
```bash
curl -N -X GET "http://localhost:8001/api/chat/chat-stream?message=请介绍一下贵州茅台"
```

**流式查询股票数据**:
```bash
curl -N -X GET "http://localhost:8001/api/chat/chat-stream?message=查询600519最近5天的数据"
```
...
```bash
curl -N -X POST http://localhost:8001/api/chat/chat-stream \
  -H "Content-Type: application/json" \
  -d '{"message": "你好"}'
```
...
# 1. 健康检查
echo "【测试 1】健康检查"
curl -s http://localhost:8001/api/chat/health
echo -e "\n"

# 2. 普通对话 (GET)
echo "【测试 2】普通对话 (GET)"
curl -s "http://localhost:8001/api/chat/chat?message=你好" | python -m json.tool
echo -e "\n"

# 3. 普通对话 (POST)
echo "【测试 3】普通对话 (POST)"
curl -s -X POST http://localhost:8001/api/chat/chat \
  -H "Content-Type: application/json" \
...
Invoke-RestMethod -Uri "http://localhost:8001/api/chat/health" | ConvertTo-Json
Write-Host ""

# 2. 普通对话 (GET)
Write-Host "【测试 2】普通对话 (GET)" -ForegroundColor Yellow
$body = @{ message = "你好" }
$response = Invoke-RestMethod -Uri "http://localhost:8001/api/chat/chat?message=$($body.message)"
...
$response = Invoke-RestMethod -Uri "http://localhost:8001/api/chat/chat" `
  -Method POST `
...
$response = Invoke-WebRequest -Uri "http://localhost:8001/api/chat/chat-stream?message=你好"
$response.Content
Write-Host ""
...
```bash
curl -N -X GET "http://localhost:8001/api/chat/chat-stream?message=请详细分析一下贵州茅台最近一周的走势"
```
...
```bash
# 显示请求和响应头
curl -v -X GET "http://localhost:8001/api/chat/chat?message=你好"

# 只显示响应头
curl -I -X GET "http://localhost:8001/api/chat/chat?message=你好"

# 测量响应时间
curl -w "\nTotal time: %{time_total}s\n" \
  -X GET "http://localhost:8001/api/chat/chat?message=你好"
```
